				...
				unsigned int dma_length)
{
	...
	edma_info->sg = (void *)((unsigned int)malloc(sgcount * sizeof(S_DRVEDMA_DESCRIPT_FORMAT)) | NON_CACHE_BIT);
	...
	DrvEDMA_SetScatterGatherTblStartAddr((E_DRVEDMA_CHANNEL_INDEX)channel, (unsigned int)edma_info->sg);
...
//	__raw_writel(1 << channel, EDMA_DISR);
	ret = DrvEDMA_InstallCallBack((E_DRVEDMA_CHANNEL_INDEX)channel, interrupt, (PFN_DRVEDMA_CALLBACK)irq_handler, (PFN_DRVEDMA_CALLBACK *)data);	
...
#ifdef EDMA_USE_IRQ
	DrvEDMA_DisableInt((E_DRVEDMA_CHANNEL_INDEX)channel, eDRVEDMA_SG | eDRVEDMA_BLKD | eDRVEDMA_TABORT);
#endif
...
int EDMA_Request(int channel)
{
	...
	DrvEDMA_EnableCH((E_DRVEDMA_CHANNEL_INDEX)channel, eDRVEDMA_ENABLE);
#ifdef EDMA_USE_IRQ
	DrvEDMA_EnableInt((E_DRVEDMA_CHANNEL_INDEX)channel, eDRVEDMA_SG | eDRVEDMA_BLKD | eDRVEDMA_TABORT);
#endif
...
#ifdef EDMA_USE_IRQ
	DrvEDMA_DisableInt((E_DRVEDMA_CHANNEL_INDEX)channel, eDRVEDMA_SG | eDRVEDMA_BLKD | eDRVEDMA_TABORT | eDRVEDMA_WAR);
#endif
	DrvEDMA_EnableCH((E_DRVEDMA_CHANNEL_INDEX)channel, eDRVEDMA_DISABLE);
...
int EDMA_SetWrapintType(int channel, int type)
{
	...
	DrvEDMA_SetWrapIntType((E_DRVEDMA_CHANNEL_INDEX)channel, type);
	if (type !=0)
	{
		DrvEDMA_DisableInt((E_DRVEDMA_CHANNEL_INDEX)channel,eDRVEDMA_SG | eDRVEDMA_BLKD | eDRVEDMA_TABORT);
