...
{
	sysprintf("SPU_SET_BASE_ADDRESS u32sysclk:%d u32SampleRate:%d \n",u32sysclk,u32SampleRate);
	DrvSPU_SetSampleRate(u32sysclk, u32SampleRate);
}

void SPU_SET_BASE_ADDRESS(UINT32 u32BaseAddress)
{
	sysprintf("SPU_SET_BASE_ADDRESS u32BaseAddress:%x\n",u32BaseAddress);
...
void spuDacOn(unsigned char  level)
{
	DacOnOffLevel = level;	
	
	sysprintf("spuDacOn level:%d\n",level);
	...
	/*Audio DAC Power Down ¹Ø±Õ*/
	outp32(REG_SPU_DAC_VOL, inp32(REG_SPU_DAC_VOL) & ~0x0800000);	//P7 power down of current bias

	sysDelay(1);
	outp32(REG_SPU_DAC_VOL, inp32(REG_SPU_DAC_VOL) & ~0x0400000);	//P6 power down of reference voltage 
	sysDelay(1);
...
VOID spuOpen(UINT32 u32SampleRate)
{
	...
	sysprintf("spuOpen u32SampleRate:%d \n",u32SampleRate);
	...
	DrvSPU_SetSrcType(0, 0x07);		// Stereo PCM16 left
...
{
	sysprintf("spuEqOpen eEqBand:%d eEqGain:%d \n",eEqBand,eEqGain);
	DrvSPU_EqOpen(eEqBand, eEqGain);
}

VOID spuEqClose (VOID)
{
	sysprintf("spuEqClose\n");
	DrvSPU_EqClose();
}
