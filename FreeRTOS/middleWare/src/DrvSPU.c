		...
		if (inp32(CH_IRQ) & u32Channel)
		{
			...
			if (u32InterruptFlag & DRVSPU_ENDADDRESS_INT)				
			{
				if ( inp32(CH_EVENT) & END_EN)
				{
					...
					//DBG_PRINTF("DRVSPU_ENDADDRESS_INT\n");
				}
			}				
			...
			if (u32InterruptFlag & DRVSPU_THADDRESS_INT)				
			{
				if ( inp32(CH_EVENT) & TH_EN)
				{
					...
					//DBG_PRINTF("DRVSPU_THADDRESS_INT\n");
				}
			}				
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		...
		return (ERRCODE)E_DRVSPU_WRONG_CHANNEL;	   	
}
...
)
{
	...
	//	DBG_PRINTF("DrvSPU_EnableInt1.0\n");
		while(inp32(CH_CTRL) & CH_FN);
	//	DBG_PRINTF("DrvSPU_EnableInt2.0\n");
		// load previous channel settings		
		outp32(CH_CTRL, (inp32(CH_CTRL) & ~CH_NO) | (u32Channel << 24));		
		outp32(CH_CTRL, inp32(CH_CTRL) | DRVSPU_LOAD_SELECTED_CHANNEL);
	//	DBG_PRINTF("DrvSPU_EnableInt3.0\n");
		while(inp32(CH_CTRL) & CH_FN);
	//	DBG_PRINTF("DrvSPU_EnableInt4.0\n");
		...
		outp32(CH_CTRL, inp32(CH_CTRL) & ~DRVSPU_UPDATE_ALL_PARTIALS);		
		outp32(CH_CTRL, inp32(CH_CTRL) | (DRVSPU_UPDATE_IRQ_PARTIAL + DRVSPU_UPDATE_PARTIAL_SETTINGS));	
	//	DBG_PRINTF("DrvSPU_EnableInt5.0\n");		
		while(inp32(CH_CTRL) & CH_FN);		
	//	DBG_PRINTF("DrvSPU_EnableInt6.0\n");	
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		//DBG_PRINTF("DrvSPU_SetPauseAddress_PCM16 u32Channel:%d u32Address:%x\n",u32Channel,u32Address);
		while(inp32(CH_CTRL) & CH_FN);
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		...
		outp32(CH_CTRL, inp32(CH_CTRL) | DRVSPU_LOAD_SELECTED_CHANNEL);
		while(inp32(CH_CTRL) & CH_FN);
	//	DBG_PRINTF("DrvSPU_GetPauseAddress_PCM16 u32Channel :%d\n",inp32(PA_ADDR));
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
	//	DBG_PRINTF("DrvSPU_SetBaseAddress u32Channel:%d u32Address:%x\n",u32Channel,u32Address);
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
		//DBG_PRINTF("DrvSPU_GetBaseAddress u32Channel:%d\n",u32Channel);
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
		//DBG_PRINTF("DrvSPU_SetThresholdAddress u32Channel:%d u32Address:%x\n",u32Channel,u32Address);
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		...
		outp32(CH_CTRL, inp32(CH_CTRL) | DRVSPU_LOAD_SELECTED_CHANNEL);
		while(inp32(CH_CTRL) & CH_FN);
		//DBG_PRINTF("DrvSPU_GetThresholdAddress u32Channel:%d\n",inp32(M_ADDR));
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
		//DBG_PRINTF("DrvSPU_SetToneAmp u32Channel:%d u32Amp:%d\n",u32Channel,u32Amp);
		// load previous channel settings		
		outp32(CH_CTRL, (inp32(CH_CTRL) & ~CH_NO) | (u32Channel << 24));		
		outp32(CH_CTRL, inp32(CH_CTRL) | DRVSPU_LOAD_SELECTED_CHANNEL);
		while(inp32(CH_CTRL) & CH_FN);
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
		//DBG_PRINTF("DrvSPU_SetTonePulse u32Channel:%d u32Pulse:%d\n",u32Channel,u32Pulse);
		// load previous channel settings		
		outp32(CH_CTRL, (inp32(CH_CTRL) & ~CH_NO) | (u32Channel << 24));		
		outp32(CH_CTRL, inp32(CH_CTRL) | DRVSPU_LOAD_SELECTED_CHANNEL);
		while(inp32(CH_CTRL) & CH_FN);
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
		//sysprintf("DrvSPU_SetEndAddress u32Channel:%d u32Address:%x\n",u32Channel,u32Address);
		// load previous channel settings		
		outp32(CH_CTRL, (inp32(CH_CTRL) & ~CH_NO) | (u32Channel << 24));		
		outp32(CH_CTRL, inp32(CH_CTRL) | DRVSPU_LOAD_SELECTED_CHANNEL);
		while(inp32(CH_CTRL) & CH_FN);
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
		//sysprintf("DrvSPU_GetEndAddress u32Channel:%d \n",u32Channel);
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
		//sysprintf("DrvSPU_GetCurrentAddress u32Channel:%d \n",u32Channel);
		// load previous channel settings		
		outp32(CH_CTRL, (inp32(CH_CTRL) & ~CH_NO) | (u32Channel << 24));		
		outp32(CH_CTRL, inp32(CH_CTRL) | DRVSPU_LOAD_SELECTED_CHANNEL);
		while(inp32(CH_CTRL) & CH_FN);
...
)
{
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
		//DBG_PRINTF("DrvSPU_GetLoopStartAddress u32Channel:%d u32Address:%x\n",u32Channel,u32Address);
		// load previous channel settings		
		outp32(CH_CTRL, (inp32(CH_CTRL) & ~CH_NO) | (u32Channel << 24));		
		outp32(CH_CTRL, inp32(CH_CTRL) | DRVSPU_LOAD_SELECTED_CHANNEL);
		while(inp32(CH_CTRL) & CH_FN);
...
{
	//DBG_PRINTF("DrvSPU_GetDFA u32Channel:%d \n", u32Channel);
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
...
{
	UINT32 u32PAN;
	//DBG_PRINTF("DrvSPU_SetPAN u32Channel:%d u16PAN:%d\n", u32Channel,u16PAN);
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
...
)
{
	...
	//DBG_PRINTF("DrvSPU_GetPAN u32Channel:%d \n", u32Channel);
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
...
/*
Sign-Magnitude, special code embedded 
.000 = 4-bit MDPCM, default 
.001 = LP8 
.011 = PCM16 
.100 = Tone 
.Two's complement, Raw PCM Data 
.101 = Mono PCM16 
.110 = Stereo PCM16 Left ([15:0]) 		立体声
.111 = Stereo PCM16 Right ([31:16]) 
*/
ERRCODE 
DrvSPU_SetSrcType(
	UINT32 u32Channel, 
...
{
	//DBG_PRINTF("DrvSPU_SetSrcType u32Channel:%d u8DataFormat:%d \n", u32Channel,u8DataFormat);
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
...
{
	UINT8 u8DataFormat;
	//DBG_PRINTF("DrvSPU_GetSrcType u32Channel:%d \n", u32Channel);
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
...
{
	UINT32 u32PAN;
	//DBG_PRINTF("DrvSPU_SetChannelVolume u32Channel:%d u8Volume:%d\n", u32Channel,u8Volume);
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
...
{
	UINT16 u32PAN;
	//DBG_PRINTF("DrvSPU_GetChannelVolume u32Channel:%d\n", u32Channel);
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
...
{
	//DBG_PRINTF("DrvSPU_EqOpen eEqBand:%d eEqGain:%d\n",  eEqBand,eEqGain);
	switch (eEqBand)
	{
...
)
{
	...
	//DBG_PRINTF("DrvSPU_SetSampleRate u32SystemClock:%d  SampleRate:%d \n",u32SystemClock,SampleRate);
	u32Buf2 = inp32(CH_EN);
//	outp32(CH_EN, 0x00);
	outp32(REG_AHBCLK, inp32(REG_AHBCLK) | ADO_CKE | SPU_CKE);			// enable SPU engine clock ，Audio DAC Engine Clock Enable 
	...
	//设置ADO_CLK 为SampleRate
	u32Buf = inp32(REG_CLKDIV1);
	u32Buf &= ~ADO_S;							//选择 ADO_SrcCLK 源
	u32Buf |= (0x3 << 19) | (u32ClkSel <<16);	//选择ADO_SrcCLK = UCLKOut 
	u32Buf &= ~ADO_N1;
	...
	for (ii=0; ii<jj; ii++)
	{
		outp32(REG_SPU_DAC_VOL, inp32(REG_SPU_DAC_VOL) - 0x0101);	// set L/R volume to zero			
		delay_loop(240);											// delay 1mS for 240MHz PLL output
//		sysDelay(1);		
	}			
	outp32(REG_CLKDIV1, u32Buf);									//Audio DAC Engine Clock Divide		
	
	for (ii=0; ii<jj; ii++)
	{
		outp32(REG_SPU_DAC_VOL, inp32(REG_SPU_DAC_VOL) + 0x0101);	// 恢复 set volume to zero			
		delay_loop(240);	// delay 1mS for 240MHz PLL output		
//		sysDelay(1);		
	}			
...
{
	//DBG_PRINTF("DrvSPU_UploadChannelContents u32Channel:%d \n",u32Channel);
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
		// wait to finish previous channel settings
		while(inp32(CH_CTRL) & CH_FN);
...
{
	//DBG_PRINTF("DrvSPU_ChannelPause u32Channel:%d\n",u32Channel);
	if ( ((INT)u32Channel >=eDRVSPU_CHANNEL_0) && ((INT)u32Channel <=eDRVSPU_CHANNEL_31) )
	{
...
{
	//DBG_PRINTF("DrvSPU_ChannelResume u32Channel:%d\n",u32Channel);
