...
{	 
	...
	unsigned int		bpb_location;
	unsigned int		fat_location;
	unsigned int		rootdir_location;
	unsigned int		free_size;
	unsigned short		rootdirentryno;
	unsigned short		totalcluster;
	unsigned short		NumCyl;
	unsigned char		NumHead;
	unsigned char		NumSector;
	unsigned char		capacity;
	unsigned char		fatcopyno;
	unsigned char		fatsectorno;
	unsigned char		fat16flg;
} Disk_Par_Info_New;
	
typedef struct
	{
	unsigned int	Mass_Base_Addr;
	unsigned int	Storage_Base_Addr;
	unsigned int	Storage_Base_Addr_RAMDISK;	
	unsigned int	SizePerSector;
	unsigned int	_usbd_DMA_Dir;
	unsigned int	_usbd_Less_MPS;
	unsigned int 	USBD_DMA_LEN;
	unsigned int	bulkonlycmd;
	unsigned int  dwUsedBytes;
	unsigned int  dwResidueLen;
	unsigned int  dwCBW_flag;	
	unsigned int	TxedFlag;
	int  gTotalSectors_RAM;
	int  gTotalSectors_SD;
	int  gTotalSectors_SM;
	unsigned int  Card_STS;	
	unsigned int  preventflag;
	unsigned int  Bulk_First_Flag;
	unsigned int  card_remove_flag;
	unsigned int  card_Insert_flag;
	unsigned int  no_card_flag;
	unsigned int  MB_Invalid_Cmd_Flg;	
	Disk_Par_Info_New DDB;
	unsigned char  SenseKey ;
	unsigned char  ASC;
	unsigned char  ASCQ;
	unsigned int	Mass_LUN;
	unsigned int	F_RAM_LUN;	
	unsigned int	F_SD_LUN;
	unsigned int 	F_SM_LUN;
	unsigned int F_CDROM_LUN;
	unsigned int CD_Tracks_Base;
	int  gTotalSectors_CDROM;	
...
typedef void (*PFN_MSCD_CDROM_CALLBACK)(unsigned int * pu32address, unsigned int u32Offset, unsigned int u32LengthInByte);

/* extern functions */
void mscdSDRAM2USB_Bulk(unsigned int DRAM_Addr ,unsigned int Tran_Size);
void mscdUSB2SDRAM_Bulk(unsigned int DRAM_Addr ,unsigned int Tran_Size);
...
unsigned char mscdFlashInit(NDISK_T *pDisk, int SDsector);
unsigned char mscdFlashInitCDROM(NDISK_T *pDisk, int SDsector, PFN_MSCD_CDROM_CALLBACK pfnCallBack ,int CdromSizeInByte);
