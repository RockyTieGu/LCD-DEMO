...
#define FRAG_SIZE		(20 * 1000)
#define HALF_FRAG_SIZE	(FRAG_SIZE/2)
extern unsigned char	*_pucPlayAudioBuff;

void spuDacOn(unsigned char level);
void spuDacOff(void);
void spuStartPlay(PFN_DRVSPU_CB_FUNC *fnCallBack, unsigned char *data);
void spuStopPlay(void);
void spuIoctl(unsigned int cmd, unsigned int arg0, unsigned int arg1);
void spuOpen(unsigned int u32SampleRate);
void spuClose (void);
