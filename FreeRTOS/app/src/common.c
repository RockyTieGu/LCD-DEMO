...
#define UART0PORT 		(0)
#define UART1PORT		(0x100)
#define PANEL_WIDTH		_LCD_WIDTH					/* PANEL Width (Raw data output width for Panel Test) */
#define PANEL_HEIGHT	_LCD_HEIGHT     			/* PANEL Height (Raw data output height for Panel Test) */

#define TARGET_WIDTH	_LCD_WIDTH					/* JPEG decode output width for __PANEL_TEST__ */
#define TARGET_HEIGHT	_LCD_HEIGHT 				/* JPEG decode output height for __PANEL_TEST__ */
...
{	
	unsigned int u32FrameBuffer;		
	JPEG_INFO_T jpegInfo;
	unsigned int u32BufferSize;
	char	fill = 0xFF;
	unsigned int u32OutputOffset;
	unsigned int u32TargetWidth,u32TargetHeight;
	unsigned short u16Width,u16Height;	
	...
	if(1)
	{
		...
		/* Allocate Raw Data Buffer for Decode Operation   _VpostFrameBufferPool*/	
		if(setBufferType == VPOST)
			u32FrameBuffer =  (UINT32)_VpostFrameBufferPool | 0x80000000;
		else if (setBufferType == BUFF0)
			u32FrameBuffer =  (UINT32)LCDOSDBuffer1 | 0x80000000;
		else if(setBufferType == BUFF1)
		...
		else
		{
			...
			//	sysprintf("\tJpeg wisth is smaller than height -> Can't downscale to QVGA -> downscale to %dx%d\n",jpegInfo.jpeg_width / ratio,jpegInfo.jpeg_height / ratio);
				...
				//sysprintf("5.0..ratio u32TargetHeight%d u32TargetWidth:%d \n",u32TargetHeight,u32TargetWidth);		
			}
			else
			{
				u32TargetHeight = jpegInfo.jpeg_height;
				u32TargetWidth = jpegInfo.jpeg_width;	
				//sysprintf("6.0..ratio u32TargetHeight%d u32TargetWidth:%d \n",u32TargetHeight,u32TargetWidth);				
			}	
		}
#endif
	...
	else
	{
		...
		else{	/* alignment for YUV420 raw data */		    
			if(u16Width % 16)
					u16Width = (u16Width & 0xFFFFFFF0) + 16;
			if(u16Height % 16)
		...
		jpegIoctl(JPEG_IOCTL_SET_DECODE_DOWNSCALE, _LCD_HEIGHT,_LCD_WIDTH);
		u16Width = _LCD_WIDTH;			
		u16Height = _LCD_HEIGHT;
		...
		if(g_u32DecFormat != JPEG_DEC_PRIMARY_PLANAR_YUV){
			u32FrameBuffer =  (unsigned int) _VpostFrameBufferPool | 0x80000000;
...
char JpegDecInputWait(unsigned int u32Address,unsigned int u32Size)
{
	...
	if(g_u32BitstreamSize > g_u32IpwUsedSize)
	{
		...
		memcpy((char *)(u32Address),(char *)(((unsigned int)g_pu8JpegBuffer | 0x80000000) + g_u32IpwUsedSize), i32Size);	
...
{
	unsigned int u32MCUX, u32MCUY, u32MUC_Width,u32MCU_Height,u32MCU_Space,u32TMP;	
	unsigned int u32j,u32k, u32sizeCount = 0;	
...
//	sysprintf("\tNeed to trigger %d times\n",u32j);
	...
	{
		apBuffer[u32k] = malloc(sizeof(unsigned char) * u32MCU_Line * (u32k + 1));
		memset(apBuffer[u32k], 0, u32MCU_Space);
	//	sysprintf("\tBuffer %d address 0x%08X size 0x%08X\n",u32k ,(unsigned int)apBuffer[u32k] ,u32MCU_Line * (u32k + 1));
...
int ParsingOldJPEG(unsigned char * JPEG_Buffer,unsigned int Length, unsigned int * pu32Width, unsigned int * pu32Height, unsigned int * u32Format,char bPrimary)
{
	...
	int HuffmanSize[5] = {0};
	int i32Result = ERR_SUCCESS;	
	char bWorkaround = FALSE;
	index = 0;			
			...
			switch(LByte)	{	//May be a Marker
				...
				case 0xDB:								/* DQT Marker (Define Quantization Table) */
					...
					if(index > Length)
                		...
                		{
                			unsigned short u16Height,u16Width,end_index,Nf,HSF[3],VSF[3],i,Ci;
				...
				case 0xDA:      						/* Scan Header */
				{
					...
					end_index = index + MLength -2;
						...
						if(bWorkaround){
							...
							pu8Addr = (unsigned char *)JPEG_Buffer;		
