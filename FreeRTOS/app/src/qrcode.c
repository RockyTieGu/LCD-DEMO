...
{
	int i, j,nDataCwIndex;
	int ncLength,ncDataCodeWord,nEncodeVersion,ncTerminater;
	unsigned char byPaddingCode;
	int ncBlock1,ncBlock2,ncBlockSum,nBlockNo,ncDataCw1,ncDataCw2,ncRSCw1,ncRSCw2;
	...
	// "0000"
	ncDataCodeWord = QR_VersonInfo[m_nVersion].ncDataCodeWord[nLevel];
...
//	ZeroMemory(m_byAllCodeWord, m_ncAllCodeWord);
	for(i=0;i<m_ncAllCodeWord;i++)m_byAllCodeWord[i]=0;
	...
	ncDataCw2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[nLevel].ncDataCodeWord;
	...
	//	ZeroMemory(m_byRSWork, sizeof(m_byRSWork));
		for(j=0;j<MAX_CODEBLOCK;j++)m_byRSWork[j]=0;
	//	memmove(m_byRSWork, m_byDataCodeWord + nDataCwIndex, ncDataCw1);
		for(j=0;j<ncDataCw1;j++)m_byRSWork[j]=*(m_byDataCodeWord + nDataCwIndex+j);
	...
	//	memmove(m_byRSWork, m_byDataCodeWord + nDataCwIndex, ncDataCw2);
		for(j=0;j<ncDataCw2;j++)m_byRSWork[j]=*(m_byDataCodeWord + nDataCwIndex+j);
...
char EncodeSourceData(char* lpsSource, int ncLength, int nVerGroup)
{
	...
	int ncSrcBits, ncDstBits,ncJoinFront, ncJoinBehind,nJoinPosition,ncComplete=0;
	int nBlock = 0;
	unsigned short wBinCode;

//	ZeroMemory(m_nBlockLength, sizeof(m_nBlockLength));
	for(j=0;j<MAX_DATACODEWORD;j++)m_nBlockLength[j]=0;
...
//	ZeroMemory(m_byDataCodeWord, MAX_DATACODEWORD);
	for(i=0;i<MAX_DATACODEWORD;i++)m_byDataCodeWord[i]=0;
	for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i){
		if (m_byBlockMode[i] == QR_MODE_NUMERAL){
			...
			for (j = 0; j < m_nBlockLength[i] / 2; ++j){
				wBinCode = KanjiToBinaly((unsigned short)(((unsigned char)lpsSource[ncComplete + (j * 2)] << 8) + (unsigned char)lpsSource[ncComplete + (j * 2) + 1]));
...
{
	int i, j;
	unsigned char nExpFirst,nExpElement;
	
	for (i = 0; i < ncDataCodeWord ; ++i){
		if (lpbyRSWork[0] != 0){
			nExpFirst = byIntToExp[lpbyRSWork[0]]; // 

			for (j = 0; j < ncRSCodeWord; ++j){
				nExpElement = (unsigned char)(((int)(byRSExp[ncRSCodeWord][j] + nExpFirst)) % 255);
...
void FormatModule()
{
	int i, j,nMinPenalty,nPenalty;

//	ZeroMemory(m_byModuleData, sizeof(m_byModuleData));
	for(i=0;i<MAX_MODULESIZE;i++)
		for(j=0;j<MAX_MODULESIZE;j++)m_byModuleData[i][j]=0;
	SetFunctionModule();
	SetCodeWordPattern();
...
{
	unsigned int nFormatInfo,nFormatData;
	unsigned int i;
...
/////////////////////////////////////////////////////////////////////////////
unsigned int CountPenalty()
{
	unsigned int nPenalty = 0;
	...
	nPenalty += (unsigned int)((ABS(50 - ((nCount * 100) / (m_nSymbleSize * m_nSymbleSize))) / 5) * 10);
...
void DrawQRCode(char* qrc)
{
	...
	if(!EncodeData(QR_LEVEL_M, QR_VRESION_S, TRUE,0, qrc, strlen(qrc)))
		return;
		
	/*LCD_FillRect(20, 20, m_nSymbleSize*4+20, m_nSymbleSize*4+20, GUI_WHITE, BUFF0);
	for(i=0;i<m_nSymbleSize;i++){
		for(j=0;j<m_nSymbleSize;j++){
			if(m_byModuleData[i][j])
				LCD_FillRect(20+i*4, 20+j*4, 24+i*4, 24+j*4, GUI_BLACK, BUFF0);
			}			
		}
