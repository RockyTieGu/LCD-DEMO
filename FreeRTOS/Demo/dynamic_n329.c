...
uint32_t *pulCounter;
	...
	for( ;; )
	{
		...
		{
			sysprintf("TaskName=%s call suspend\n", pcTaskGetTaskName( NULL ));
			vTaskSuspend( NULL );
		}
	}
}
...
short sError = pdFALSE;
	...
	for( ;; )
	{
		...
		for( sLoops = 0; sLoops < priLOOPS; sLoops++ )
		{
			...
			priority. */
			sysprintf("TaskName=%s suspend CNT_INC task\n", pcTaskGetTaskName( NULL ));
			...
			sysprintf("TaskName=%s resume CNT_INC task\n", pcTaskGetTaskName( NULL ));
		...
		sysprintf("TaskName=%s resume LIM_INC task\n", pcTaskGetTaskName( NULL ));
...
static uint32_t ulValueToSend = ( uint32_t ) 0;
	...
	for( ;; )
	{
		...
		sysprintf("TaskName=%s call delay time=%d\n", pcTaskGetTaskName( NULL ), priSLEEP_TIME);
		vTaskDelay( priSLEEP_TIME );
