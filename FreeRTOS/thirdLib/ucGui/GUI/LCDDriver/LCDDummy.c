  ...
  if ((GUI_Context.DrawMode & LCD_DRAWMODE_TRANS) == 0) {
    if (pTrans) {
      for (; xsize > 0; xsize--, x++, p++) {
        pixel = *p;
        //LCD_L0_SetPixelIndex(x, y, *(pTrans + pixel));
		//LCDOSDBuffer1[y][x] = *(pTrans + pixel);
		   *(_VpostFrameBuffer+(y)*_LCD_WIDTH+x) = *(pTrans + pixel);
      }
    } else {
      for (;xsize > 0; xsize--, x++, p++) {
        //LCD_L0_SetPixelIndex(x, y, *p);
        /// LCDOSDBuffer1[y][x] = *p;
		  *(_VpostFrameBuffer+(y)*_LCD_WIDTH+x) =  *p;
      }
    }
  } else {
    if (pTrans) {
      ...
      for (; xsize > 0; xsize--, x++, p++) {
        ...
        if (pixel) {
       //   LCD_L0_SetPixelIndex(x, y, pixel);
			//LCDOSDBuffer1[y][x] = pixel;
			  *(_VpostFrameBuffer+(y)*_LCD_WIDTH+x) = pixel;
        }
      }
    }
  }
...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex) {
  ...
  {
//    TFTDotWrite(x,y,PixelIndex);/*画点函数，例程自行添加的*/      /* ... */
//	  LCD_SetPoint(x,y,PixelIndex);
	//  LCDOSDBuffer1[y][x]=PixelIndex;
...
int  LCD_L0_Init(void) 
{
   ...
   lcdformatex.ucVASrcFormat = DRVVPOST_FRAME_RGB565;
#endif
    lcdformatex.nScreenWidth = LCD_XSIZE;
	lcdformatex.nScreenHeight = LCD_YSIZE;
