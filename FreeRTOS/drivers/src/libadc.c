...
int adc_read_init(unsigned char mode)
{
	...
	if(_opened != 2)return(-1);//确认adc已打开
				
	if(mode != ADC_NONBLOCK && mode != ADC_BLOCK)//参数不对返回 	
	{
		sysDisableInterrupt(IRQ_ADC); 	
		return(-1);
	}		
...
///////////////////////////////////////////////	
    ...
    {			
			if( (inp32(REG_ADC_CON) & WT_INT) != WT_INT) return 0;	//没有触屏中断 返回
		//	tp_f=0;
...
/////////////////////////////////////////////	
	/* set to Auto mode */
	/*0211 */ 
	adc_disableInt(eADC_WT_int);////关ADC WT中断
	/*0211 */ 
...
/*0211 */  	
    adc_enableInt(eADC_ADC_int);	//关ADC中断
/*0211 */    

	g_bIsADCInt = 0;
	DrvADC_Conversion();	//转换
#ifdef USING_INT		
	while(g_bIsADCInt==0);				
#else
	DrvADC_PollingADC( );//等待转换完成 丢弃 第一次的数据
#endif									
	...
	for(u32Idx = 0; u32Idx<SORT_FIFO; u32Idx=u32Idx+1)	//6次
	{	
		...
		while(g_bIsADCInt==0);
#else
		DrvADC_PollingADC( );//等待转换完成
#endif		
		DrvADC_GetTscData(&au16XPos[u32Idx],&au16YPos[u32Idx]);	//得到数据
	}																
		
	    quicksort(au16XPos, 0, SORT_FIFO-1); //排顺序
    	quicksort(au16YPos, 0, SORT_FIFO-1); 
          	
	*x = (au16XPos[SORT_FIFO-3]+au16XPos[SORT_FIFO-2])/2;//去除最大最小平均 
	*y = (au16YPos[SORT_FIFO-3]+au16YPos[SORT_FIFO-2])/2;	
	
	bIsXnear = ((i32_x-60) <*x ) && (*x <(i32_x+60)); 
	bIsYnear = ((i32_y-60) <*y ) && (*y <(i32_y+60)); //16 改 60
