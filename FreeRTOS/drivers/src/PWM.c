...
{ 
    unsigned int volatile u32RegPIIR,u32RegCCR0,u32RegCCR1;
...
/*---------------------------------------------------------------------------------------------------------*/
char PWM_IsTimerEnabled(unsigned char u8Timer)
{
...
/*---------------------------------------------------------------------------------------------------------*/
void PWM_SetTimerCounter(unsigned char u8Timer, unsigned short u16Counter)
{
...
/*---------------------------------------------------------------------------------------------------------*/
unsigned int PWM_GetTimerCounter(unsigned char u8Timer)
{
...
)
{
	...
	sysInstallISR(IRQ_LEVEL_1, IRQ_PWM, (void *)PWM_ISR);	
...
/*---------------------------------------------------------------------------------------------------------*/
void PWM_EnableInt(unsigned char u8Timer, unsigned char u8Int)
{
	if(u8Timer & 0x10)
	{		
		if(u8Timer & 0x02)		
...
/*---------------------------------------------------------------------------------------------------------*/
void PWM_DisableInt(unsigned char u8Timer, unsigned char u8Int)
{   
	if(u8Timer & 0x10)
	{
		if(u8Timer & 0x02)		
...
/*               This function is used to clear the PWM timer/capture interrupt 		                   */
/*---------------------------------------------------------------------------------------------------------*/
void PWM_ClearInt(unsigned char u8Timer)
{
	if(u8Timer & 0x10)
	{
		if(u8Timer & 0x02)
			outp32(CCR1, inp32(CCR1) & ~(1 << (((u8Timer & 0x01) << 4) + 4)));
		else
			outp32(CCR0, inp32(CCR0) & ~(1 << (((u8Timer & 0x01) << 4) + 4)));			
	}
	else
		outp32(PIIR,  inp32(PIIR) & ~(1<<(u8Timer & 0x03)));
}
...
/*---------------------------------------------------------------------------------------------------------*/
char PWM_GetIntFlag(unsigned char u8Timer)
{
	if(u8Timer & 0x10)
	{
		if(u8Timer & 0x02)
...
/*				 transition		                   														   */
/*---------------------------------------------------------------------------------------------------------*/
unsigned short PWM_GetRisingCounter(unsigned char u8Capture)
{
...
/*				 transition		                   														   */
/*---------------------------------------------------------------------------------------------------------*/
unsigned short PWM_GetFallingCounter(unsigned char u8Capture)
{
...
/*---------------------------------------------------------------------------------------------------------*/
char PWM_GetCaptureIntStatus(unsigned char u8Capture, unsigned char u8IntType)
{	
	if(u8Capture & 0x02)
	{
...
/*---------------------------------------------------------------------------------------------------------*/
void PWM_ClearCaptureIntStatus(unsigned char u8Capture, unsigned char u8IntType)
{
	if(u8Capture & 0x02)
		outp32(CCR1, inp32(CCR1) & ~(1 << (((u8Capture & 0x01) << 4) + u8IntType)));
	else
		outp32(CCR0, inp32(CCR0) & ~(1 << (((u8Capture & 0x01) << 4) + u8IntType)));
}
...
/*---------------------------------------------------------------------------------------------------------*/
void PWM_EnableDeadZone(unsigned char u8Timer, unsigned char u8Length,char bEnableDeadZone)
{
	if(u8Timer & 0x02)
	{
...
/*---------------------------------------------------------------------------------------------------------*/
void PWM_Enable(unsigned char u8Timer,char bEnable)
{
...
/*---------------------------------------------------------------------------------------------------------*/
FLOAT PWM_SetTimerClk(unsigned char u8Timer, PWM_TIME_DATA_T *sPt)
{
	FLOAT 	fFrequency;
	unsigned short	u16Duty;
	unsigned int u32PllKHz, u32SysKHz, u32CpuKHz, u32HclkKHz, u32ApbKHz;
	...
	else
	{
		...
		u16Duty = (unsigned short )(fFrequency/u16PreScale/u8Divider);
...
/*---------------------------------------------------------------------------------------------------------*/
void PWM_SetTimerIO(unsigned char u8Timer, char bEnable)
{
