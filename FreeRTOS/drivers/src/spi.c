...
/*-----------------------------------------------------------------------------------*/
static char _spi_init_flag0 = FALSE, _spi_init_flag1 = FALSE;
...
void spiIoctl(int spiPort, int spiFeature, int spiArg0, int spiArg1)
{
	switch(spiFeature)
	{
		case SPI_SET_CLOCK:
	...
	switch (RxBitLen)
	{
		case SPI_32BIT:
			...
			while (len-- > 0)
			{
				...
				spiActive(port);
				if (port == 0)
					*((int *)pDst) = inpw(REG_SPI0_RX0) & 0xffffffff;
				else
					*((int *)pDst) = inpw(REG_SPI1_RX0) & 0xffffffff;
				pDst += 4;
			}
		...
		case SPI_16BIT:
			...
			while (len-- > 0)
			{
				...
				spiActive(port);
				if (port == 0)
					*((short *)pDst) = inpw(REG_SPI0_RX0) & 0xffff;
				else
					*((short *)pDst) = inpw(REG_SPI1_RX0) & 0xffff;
				pDst += 2;
			}
		...
		case SPI_8BIT:
			...
			while (len-- > 0)
			{
				...
				spiActive(port);
				if (port == 0)
					*((char *)pDst) = inpw(REG_SPI0_RX0) & 0xff;
				else
					*((char *)pDst) = inpw(REG_SPI1_RX0) & 0xff;
				pDst ++;
			}
...
int spiWrite(int port, int TxBitLen, int len, CHAR *pSrc)
{
	switch (TxBitLen)
	{
		case SPI_32BIT:
			...
			{
				if (port == 0)
					outpw(REG_SPI0_TX0, *((int *)pSrc));
				else
					outpw(REG_SPI1_TX0, *((int *)pSrc));
				spiTxLen(port, 0, SPI_32BIT);
				spiActive(port);
				pSrc += 4;
			}
		...
		case SPI_16BIT:
			...
			{
				if (port == 0)
					outpw(REG_SPI0_TX0, *((short *)pSrc));
				else
					outpw(REG_SPI1_TX0, *((short *)pSrc));
				spiTxLen(port, 0, SPI_16BIT);
				spiActive(port);
				pSrc += 2;
			}
		...
		case SPI_8BIT:
			...
			{
				if (port == 0)
					outpw(REG_SPI0_TX0, *((char *)pSrc));
				else
					outpw(REG_SPI1_TX0, *((char *)pSrc));
				spiTxLen(port, 0, SPI_8BIT);
				spiActive(port);
				pSrc ++;
			}
